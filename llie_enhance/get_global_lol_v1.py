...
model = IAT().cuda()
s = "/home/wsz/workspace/Illumination-Adaptive-Transformer/IAT_enhance/best_Epoch_lol_v1.pth"
...
if config.save:
    ...
    result_path_mid = config.img_val_path.replace('low', 'Result_mid')
    mkdir(result_path)
    mkdir(result_path_mid)
...
with torch.no_grad():
    for i, imgs in tqdm(enumerate(val_loader)):
        ...
        mid_img = (low_img.mul(mul)).add(add)

        if config.save:
            torchvision.utils.save_image(enhanced_img, result_path + str(name) + '.png')
            torchvision.utils.save_image(mid_img, result_path_mid + str(name) + '.png')
