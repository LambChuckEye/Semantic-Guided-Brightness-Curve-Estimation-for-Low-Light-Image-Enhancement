...
model = DecomNet().cuda()
s = "/home/wsz/workspace/Illumination-Adaptive-Transformer/IAT_enhance/workdirs/snapshots_folder_lol_v1_patch_decom_0.2_221019/best_Epoch.pth"
...
if config.save:
    result_path = config.img_val_path.replace('low', 'Result_decom')
    mkdir(result_path)
...
with torch.no_grad():
    for i, imgs in tqdm(enumerate(val_loader)):
        ...
        print(name)
        #print(low_img.shape)
        R_low, I_low = model(low_img)
        R_high, I_high = model(high_img)
        if config.save:
            torchvision.utils.save_image(R_low, result_path + str(name) + '_R_low.png')
            torchvision.utils.save_image(I_low, result_path + str(name) + '_I_low.png')
            torchvision.utils.save_image(R_high, result_path + str(name) + '_R_high.png')
            torchvision.utils.save_image(I_high, result_path + str(name) + '_I_high.png')

        ssim_value = ssim(R_low, R_high, as_loss=False).item()
        psnr_value = psnr(R_low, R_high).item()
